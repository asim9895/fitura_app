...
import { remove_selected_date_water_data } from "@/api/water_apis";
...
interface SingleStepItemProps {
  ...
  fetch_all_waters_data: () => Promise<void>;
}

const SingleWaterItem: React.FC<SingleStepItemProps> = ({
  water,
  fetch_all_waters_data,
    ...
    >
      ...
      >
        ...
        >
          ...
          >
            Steps: <Text style={{ color: colors.button }}>{water.intake}</Text>
          </Text>
        </View>
      </View>
      <TouchableOpacity
        onPress={async () => {
          await remove_selected_date_water_data(selected_date, water.id);
          await fetch_selected_date_water_data(selected_date);
          await fetch_all_waters_data();
        }}
      >
        <Image
...
export default SingleWaterItem;
